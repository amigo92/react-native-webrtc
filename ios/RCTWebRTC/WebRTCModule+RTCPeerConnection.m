                  ...
                  objectID:(nonnull NSNumber *)objectID)
{
    ...
    self.bytesNow[objectID] = 0;
    self.bytesBefore[objectID] = 0;
    self.tsNow[objectID] = 0;
    self.tsBefore[objectID] = 0;
    self.bitRate[objectID] = 0;
}
...
RCT_EXPORT_METHOD(peerConnectionClose:(nonnull NSNumber *)objectID)
{
    ...
    [self.bytesNow removeObjectForKey:objectID];
    [self.tsNow removeObjectForKey:objectID];
    [self.bytesBefore removeObjectForKey:objectID];
    [self.tsBefore removeObjectForKey:objectID];
    [self.bitRate removeObjectForKey:objectID];
...
RCT_EXPORT_METHOD(setMonitor:(nonnull NSString *)trackID
                  objectID:(nonnull NSNumber *)objectID)
{
    
    [NSTimer scheduledTimerWithTimeInterval:5.0
                                     target:self
                                   selector:@selector(checkBitrate:)
                                   userInfo:@{@"trackID":trackID,@"objectID": objectID}
                                    repeats:YES];
}

RCT_EXPORT_METHOD(getMonitor:(nonnull NSString *)trackID
                  objectID:(nonnull NSNumber *)objectID
                  callback:(RCTResponseSenderBlock)callback)
{
    callback(@[
               (self.bitRate[objectID].stringValue)
               ]);
}

- (void)checkBitrate:(NSTimer *)timer {
    NSString *trackID = timer.userInfo[@"trackID"];
    NSNumber *objectID = timer.userInfo[@"objectID"];
    ...
    if (!trackID
        ...
        [peerConnection statsForTrack:track
                    ...
                    completionHandler:^(NSArray<RTCLegacyStatsReport *> *stats) {
                        ...
                        NSLog(@"pc: %@  stats: %@",trackID,@[[self statsToJSON:stats]]);
                        NSNumberFormatter *f = [[NSNumberFormatter alloc] init];
                        f.numberStyle = NSNumberFormatterDecimalStyle;
                        for (RTCLegacyStatsReport *report in stats) {
                            NSRange rangeValue = [report.reportId rangeOfString:@"recv" options:NSCaseInsensitiveSearch];
                            ...
                            [report.values enumerateKeysAndObjectsUsingBlock:^(
                                                                               ...
                                                                               NSString *value,
                                                                               BOOL *stop) {
                                if ([key  isEqual: @"bytesReceived"]) {
                                    NSNumber *bytes = [f numberFromString:value];
                                    if ( self.bytesNow[objectID] == nil ) {
                                        self.bytesNow[objectID] = bytes;
                                        NSLog(@"bytesNow for %f: %f", objectID.floatValue, (self.bytesNow[objectID]).floatValue);
                                        self.tsNow[objectID] = [NSNumber numberWithDouble: report.timestamp];
                                    }
                                    else{
                                        NSNumber *bytes = [f numberFromString:value];
                                        self.bytesNow[objectID] = [NSNumber numberWithFloat:([self.bytesNow[objectID] floatValue] + [bytes floatValue])];
                                        self.tsNow[objectID] = [NSNumber numberWithDouble: report.timestamp];
                                    }
                                }
                            }];
                        }
                        NSNumber *bitsDiff = [NSNumber numberWithFloat:([self.bytesNow[objectID] floatValue] - [self.bytesBefore[objectID] floatValue])];
                        NSLog(@"bits diff for %f: %@",objectID.floatValue, bitsDiff.stringValue);
                        NSNumber *timeDiff = [NSNumber numberWithDouble:([self.tsNow[objectID] doubleValue] - [self.tsBefore[objectID] doubleValue])];
                        NSLog(@"time diff for %f: %@",objectID.floatValue, timeDiff.stringValue);
                        self.bitRate[objectID] = [NSNumber numberWithDouble: (bitsDiff.floatValue/timeDiff.doubleValue)*8];
                        NSLog(@"Kb/s for %f: %f",objectID.floatValue, (bitsDiff.floatValue/timeDiff.doubleValue)*(8));
                        self.bytesBefore[objectID] = self.bytesNow[objectID];
                        self.tsBefore[objectID] = [NSNumber numberWithDouble: self.tsNow[objectID].doubleValue];
                    }];
    }
}
